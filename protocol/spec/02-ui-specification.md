...
| `T_Modified` | `T` + `T` | Open **Time modification mode** modal to edit the current bout (period) time — only when the bout timer is stopped; enter new remaining time in M:SS format; `Enter` confirms, `Escape` cancels |
